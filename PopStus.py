...
class Pop():
    ...
    def _process_file(self, csv_file):
        ...
        area_match = re.search(r"\\漂移清洗\\([^\\]+)", csv_file)
        if area_match:
        ...
        new_points = new_points[['ID','timestamp','FID']]
        # 时间转码
        new_points['timestamp'] = pd.to_datetime(new_points['timestamp'], format='%Y-%m-%d %H:%M:%S')
        new_points["time"] = new_points["timestamp"].dt.hour * 4 + new_points["timestamp"].dt.minute // 15
        # 聚类统计区域的人流量
        new_points.drop('timestamp', axis=1, inplace=True) # 直接修改原 DataFrame
